-- Create tables for the data engineering project (PostgreSQL version)
...
CREATE INDEX IF NOT EXISTS idx_sales_customer_id ON sales_records(customer_id);
CREATE INDEX IF NOT EXISTS idx_sales_date ON sales_records(sale_date);
CREATE INDEX IF NOT EXISTS idx_sales_category ON sales_records(product_category);
CREATE INDEX IF NOT EXISTS idx_sales_location ON sales_records(store_location);

-- Insert a test record to verify table creation (PostgreSQL syntax)
